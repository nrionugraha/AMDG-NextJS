        ...
        <>
          ...
          <main className="profile-container">
            <section className="profile-card">
              <h2 className="user-name">Username:
                <br /> {username}
              </h2>
              <p id="walletAddress" className="wallet-address">Wallet: {wallet}</p>
            </section>
                  ...
                  <div className="tweet-comments">
                    {tweet.comments.map(comment => (
                      <p key={comment.id} className="tweet-comment">
                        ...
                        <span className="tweet-meta">{comment.author} - {new Date(comment.timestamp * 1000).toLocaleString()}</span>: {comment.text}
                      </p>
                    ))}
                  </div>
                </div>
              ...
              <div>
                ...
                {myComments.map(comment => (
                  <div key={comment.id} className="card">
                    <p><strong>Comment on {comment.username}'s tweet</strong></p>
                    <p>{comment.text}</p>
                    <p style={{ fontSize: '0.8em', color: 'gray' }}>{new Date(comment.timestamp * 1000).toLocaleString()}</p>
                  </div>
                ))}
